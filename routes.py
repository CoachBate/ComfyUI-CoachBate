...
from .monkeypatch_settings import load_monkeypatch_settings, save_monkeypatch_settings
from .workflow_path_autofix import parse_override_line, save_override_rows
...
_MONKEYPATCH_SETTINGS_FILE = os.path.join(
    os.path.dirname(__file__), "monkeypatch_settings.json"
)


@routes.get("/coachbate/monkeypatch_settings")
async def get_monkeypatch_settings(request):
    """Return the server-wide third-party compatibility-patch settings."""
    return web.json_response({
        "settings": load_monkeypatch_settings(_MONKEYPATCH_SETTINGS_FILE),
    })


@routes.post("/coachbate/monkeypatch_settings")
async def update_monkeypatch_settings(request):
    """Persist compatibility-patch settings for the next ComfyUI start."""
    try:
        body = await request.json()
        if not isinstance(body, dict):
            raise ValueError("request body must be an object")
        settings = save_monkeypatch_settings(
            _MONKEYPATCH_SETTINGS_FILE,
            body.get("settings"),
        )
        ...
        log.error("[CoachBate] Failed to save monkeypatch settings: %s", exc)
        return web.json_response({"error": str(exc)}, status=500)
...
async def workflow_path_autofix_overrides(request):
    ...
    Format (one rule per line): search<WHITESPACE>replacement
...
@routes.post("/coachbate/workflow_path_autofix/overrides")
async def save_workflow_path_autofix_overrides(request):
    """Validate and persist model-path override rules from global settings."""
    try:
        body = await request.json()
        if not isinstance(body, dict):
            raise ValueError("request body must be an object")
        normalized = save_override_rows(_OVERRIDES_FILE, body.get("overrides"))
        overrides = [
            {"search": search, "replacement": replacement}
            for search, replacement in normalized
        ]
        log.info("[CoachBate] Saved %d workflow path override rules", len(overrides))
        return web.json_response({"ok": True, "overrides": overrides})
    except (ValueError, TypeError) as exc:
        return web.json_response({"error": str(exc)}, status=400)
    except Exception as exc:
        log.error("[CoachBate] Failed to save workflow path overrides: %s", exc)
        return web.json_response({"error": str(exc)}, status=500)
...
_CUSTOM_NODES_DIR = os.path.normpath(os.path.join(os.path.dirname(__file__), ".."))


def _is_workflow_json(path):
    """Return True if the JSON file looks like a ComfyUI workflow (has a 'nodes' list)."""
    try:
        with open(path, "r", encoding="utf-8", errors="ignore") as fh:
            data = json.load(fh)
...
@routes.get("/coachbate/custom_nodes/listing")
async def custom_nodes_listing(request):
    """
    List one level of the custom_nodes directory (non-recursive).

    Query params:
      folder=<cn-relative path>  optional; default "" = custom_nodes root

    Returns { "folders": ["ComfyUI-Foo", ...], "files": [{path, size, modified, created}, ...] }
    where files are only .json files that contain a ComfyUI "nodes" list.
    Folder paths are relative to the custom_nodes root.
    """
    folder_param = request.query.get("folder", "").strip().strip("/")
    if folder_param:
        scan_dir = os.path.normpath(os.path.join(_CUSTOM_NODES_DIR, folder_param))
        if os.path.commonpath([scan_dir, _CUSTOM_NODES_DIR]) != _CUSTOM_NODES_DIR:
            return web.json_response({"error": "invalid folder"}, status=403)
    else:
        scan_dir = _CUSTOM_NODES_DIR
    ...
    try:
        names = sorted(os.listdir(scan_dir), key=str.lower)
    except OSError as exc:
        return web.json_response({"error": str(exc)}, status=500)
    ...
    for name in names:
        if name.startswith(".") or name.startswith("__"):
            continue
        abs_path = os.path.join(scan_dir, name)
        if os.path.isdir(abs_path):
            rel = os.path.relpath(abs_path, _CUSTOM_NODES_DIR).replace(os.sep, "/")
            folders.append(rel)
        elif name.lower().endswith(".json"):
            is_wf = await loop.run_in_executor(None, _is_workflow_json, abs_path)
            if is_wf:
                rel = os.path.relpath(abs_path, _CUSTOM_NODES_DIR).replace(os.sep, "/")
                try:
                    st = os.stat(abs_path)
...
@routes.get("/coachbate/custom_nodes/file")
async def custom_nodes_file(request):
    """Serve a custom_nodes JSON file so the frontend can open it as a workflow."""
    path_param = request.query.get("path", "").strip().strip("/")
    if not path_param:
        return web.json_response({"error": "path required"}, status=400)
    abs_path = os.path.normpath(os.path.join(_CUSTOM_NODES_DIR, path_param))
    if os.path.commonpath([abs_path, _CUSTOM_NODES_DIR]) != _CUSTOM_NODES_DIR:
        return web.json_response({"error": "invalid path"}, status=403)
    if not os.path.isfile(abs_path):
        return web.json_response({"error": "not found"}, status=404)
    try:
        return web.FileResponse(abs_path, headers={"Content-Type": "application/json"})
    except Exception as exc:
        return web.json_response({"error": str(exc)}, status=500)
