...
test("flattenVisible lists a folder's own files before its subfolders at the same depth", () => {
    // Regression test: a collapsed subfolder immediately followed by
    // equally-indented files reads as "those files are inside it". Files
    // must come first so a folder row never appears to trail a file.
    const tree = buildTree(makeEntries());
    const rows = flattenVisible(tree, new Set(["MCB"]), { field: "name", dir: 1 });

    const mcbLevelRows = rows.filter((r) => r.depth === 1);
    const kinds = mcbLevelRows.map((r) => r.kind);
    const firstFolderIdx = kinds.indexOf("folder");
    const lastFileIdx = kinds.lastIndexOf("file");
    assert.ok(firstFolderIdx !== -1 && lastFileIdx !== -1);
    assert.ok(lastFileIdx < firstFolderIdx, `expected all files before any folder, got kinds=${kinds}`);
});
