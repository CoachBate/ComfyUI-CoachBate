...
def _build_guide_data(timeline_data_str, duration_frames, guide_strength_str,
                      custom_width, custom_height, resize_method, divisible_by, img_compression,
                      frame_rate):
    """Extract and resize guide images from timeline segments. Returns (guide_data, derived_w, derived_h)."""
    guide_data = {"images": [], "insert_frames": [], "strengths": [], "frame_rate": frame_rate}
    derived_w, derived_h = custom_width, custom_height
    try:
        tdata = json.loads(timeline_data_str) if timeline_data_str else {}
        img_segs = [
        ...
        if not guide_data["images"]:
            w = max(32, (derived_w if derived_w > 0 else 768) // 32 * 32)
            h = max(32, (derived_h if derived_h > 0 else 512) // 32 * 32)
...
def _slice_timeline_for_chunk(tdata, chunk_start, chunk_end, duration_frames):
    """Slice all segments in tdata to the [chunk_start, chunk_end) window and offset start times."""
    def clip_seg(s):
        s_start = int(s.get("start", 0))
        s_len = int(s.get("length", 1))
        s_end = s_start + s_len
        if s_end <= chunk_start or s_start >= chunk_end:
            return None
        new_start = max(s_start, chunk_start) - chunk_start
        new_end = min(s_end, chunk_end) - chunk_start
        result = {**s, "start": new_start, "length": new_end - new_start}
        # Advance trimStart for audio clipped from the left so chunk N picks up mid-file
        if s.get("type") == "audio" and chunk_start > s_start:
            result["trimStart"] = int(s.get("trimStart", 0)) + (chunk_start - s_start)
        return result

    sliced_segs = [r for s in tdata.get("segments", []) if (r := clip_seg(s)) is not None]
    sliced_audio = [r for s in tdata.get("audioSegments", []) if (r := clip_seg(s)) is not None]
    return {**tdata, "segments": sliced_segs, "audioSegments": sliced_audio, "chunkMarkers": []}


def _derive_chunk_prompts(tdata_sliced, fallback_local_prompts, fallback_segment_lengths, fallback_guide_strength):
    segs = sorted(tdata_sliced.get("segments", []), key=lambda s: s["start"])
    if not segs:
        return fallback_local_prompts, fallback_segment_lengths, fallback_guide_strength
    local_prompts = " | ".join(s.get("prompt", "") for s in segs)
    segment_lengths = ",".join(str(s["length"]) for s in segs)
    guide_strength = ",".join(
        str(s.get("guideStrength", 1.0)) for s in segs if s.get("type", "image") != "text"
    )
    return local_prompts, segment_lengths, guide_strength
...
class LTXDirector(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.String.Input(
                    "chunk_markers", default="",
                    tooltip="Auto-populated from the timeline editor (comma-separated chunk marker frame positions).",
                ),
                io.Int.Input(
                    "chunk_index", default=0, min=0, max=99, step=1, optional=True,
                    tooltip="0=render full video. 1, 2, 3...=render a specific chunk. Use with LTX Video Concat to stitch chunks manually.",
                ),
            ],
            outputs=[
                ...
                io.Int.Output(display_name="audio_latent_length", tooltip="Original audio latent frame count. Connect to LTX Trim Latent after the KSampler. 0 when no audio VAE is connected."),
            ],
        )
    ...
    def execute(cls, model, clip, global_prompt, duration_frames, duration_seconds,
                ...
                use_custom_audio=False, chunk_markers="", chunk_index=0) -> io.NodeOutput:

        # --- Chunk index handling: slice timeline to a specific chunk ---
        if chunk_index > 0 and timeline_data:
            try:
                tdata_raw = json.loads(timeline_data)
                raw_markers = tdata_raw.get("chunkMarkers", [])
                if not raw_markers and chunk_markers.strip():
                    raw_markers = [int(float(x.strip())) for x in chunk_markers.split(",") if x.strip()]
                sorted_markers = sorted(raw_markers)

                # Build boundaries: [0, marker1, marker2, ..., duration_frames]
                boundaries = [0] + sorted_markers + [duration_frames]
                chunk_start = boundaries[chunk_index - 1] if chunk_index - 1 < len(boundaries) else 0
                chunk_end = boundaries[chunk_index] if chunk_index < len(boundaries) else duration_frames

                if chunk_start < chunk_end:
                    chunk_duration = chunk_end - chunk_start

                    # Filter segments to those within this chunk, offset their start times
                    def clip_seg_to_chunk(s):
                        seg_start = int(s.get("start", 0))
                        seg_len = int(s.get("length", 1))
                        seg_end = seg_start + seg_len
                        if seg_end <= chunk_start or seg_start >= chunk_end:
                            return None
                        new_start = max(seg_start, chunk_start) - chunk_start
                        new_end = min(seg_end, chunk_end) - chunk_start
                        result = {**s, "start": new_start, "length": new_end - new_start}
                        if s.get("type") == "audio" and chunk_start > seg_start:
                            result["trimStart"] = int(s.get("trimStart", 0)) + (chunk_start - seg_start)
                        return result

                    sliced_segs = [r for s in tdata_raw.get("segments", []) if (r := clip_seg_to_chunk(s)) is not None]
                    sliced_audio = [r for s in tdata_raw.get("audioSegments", []) if (r := clip_seg_to_chunk(s)) is not None]

                    sliced_tdata = {**tdata_raw, "segments": sliced_segs, "audioSegments": sliced_audio, "chunkMarkers": []}
                    timeline_data = json.dumps(sliced_tdata)
                    duration_frames = chunk_duration

                    local_prompts, segment_lengths, guide_strength = _derive_chunk_prompts(
                        sliced_tdata, local_prompts, segment_lengths, guide_strength
                    )
                    log.info("[LTXDirector] Chunk %d: frames %d-%d (duration %d), %d segments",
                             chunk_index, chunk_start, chunk_end, chunk_duration, len(sliced_segs))
            except Exception as e:
                log.warning("[LTXDirector] Chunk slicing failed for chunk_index=%d: %s", chunk_index, e)

        # --- Build guide_data from image segments ---
        guide_data, derived_w, derived_h = _build_guide_data(
            timeline_data, duration_frames, guide_strength,
            custom_width, custom_height, resize_method, divisible_by, img_compression, frame_rate
        )
        ...
        return io.NodeOutput(patched, conditioning, latent, audio_latent, guide_data, float(frame_rate), audio_out, num_audio_latents)
