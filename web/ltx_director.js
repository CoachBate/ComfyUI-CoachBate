...
const HIDDEN_WIDGET_NAMES = ["timeline_data", "local_prompts", "segment_lengths", "guide_strength", "audio_data", "use_custom_audio", "chunk_markers"];
...
const STYLES = `
  ...
  .pr-photopea-toolbar {
    ...
    padding: 8px 16px; background: #1a1a1a; border-bottom: 1px solid #444;
    flex-shrink: 0;
  }
  .pr-photopea-title {
    color: #e0e0e0; font-size: 13px; font-weight: 600; font-family: inherit;
  }
  .pr-photopea-btn {
  ...
  .pr-photopea-btn:hover { background: #3a3a3a; }
  .pr-photopea-btn.save { background: #1a4a2a; border-color: #2d7a3a; color: #6eff8a; }
  .pr-photopea-btn.save:hover { background: #245a33; }
  .pr-photopea-btn.cancel { color: #aaa; }
  .pr-photopea-iframe {
...
function parseInitial(jsonStr) {
  let parsed = { segments: [], audioSegments: [], chunkMarkers: [] };
  try {
    if (jsonStr) {
      ...
      if (Array.isArray(p.chunkMarkers)) parsed.chunkMarkers = p.chunkMarkers;
    }
  } catch (e) { }
...
// Convert seconds + fps to the nearest valid LTX frame count (must satisfy 8n + 1).
// Formula: ceil(((seconds * fps) - 1) / 8) * 8 + 1
function ltxFramesFromSeconds(seconds, fps) {
  const raw = seconds * fps;
  return Math.ceil((raw - 1) / 8) * 8 + 1;
}

// Snap an arbitrary frame count to the nearest valid LTX value (8n + 1).
function snapToLtxFrames(frames) {
  return Math.round((Math.max(1, frames) - 1) / 8) * 8 + 1;
}

// --- Photopea In-Place Image Editor ---
class PhotopeaDialog {
  constructor() {
    this.overlay = null;
    this.iframe = null;
    this._onSave = null;
  ...
  open(imageUrl, onSave) {
    this._onSave = onSave;

    this.overlay = document.createElement("div");
    this.overlay.className = "pr-photopea-overlay";

    const toolbar = document.createElement("div");
    toolbar.className = "pr-photopea-toolbar";

    const title = document.createElement("span");
    title.className = "pr-photopea-title";
    title.textContent = "Edit in Photopea";

    const btnGroup = document.createElement("div");
    btnGroup.style.cssText = "display:flex;gap:8px;";

    const fullBtn = document.createElement("button");
    fullBtn.className = "pr-photopea-btn";
    fullBtn.textContent = "⛶ Fullscreen";
    fullBtn.onclick = () => {
      this._isFullscreen = !this._isFullscreen;
      fullBtn.textContent = this._isFullscreen ? "⊠ Exit Fullscreen" : "⛶ Fullscreen";
    };

    const cancelBtn = document.createElement("button");
    cancelBtn.className = "pr-photopea-btn cancel";
    cancelBtn.textContent = "Cancel";
    cancelBtn.onclick = () => this.close();

    const saveBtn = document.createElement("button");
    saveBtn.className = "pr-photopea-btn save";
    saveBtn.textContent = "Save to Timeline";
    saveBtn.onclick = () => this._save(saveBtn);

    btnGroup.append(fullBtn, cancelBtn, saveBtn);
    toolbar.append(title, btnGroup);

    this.iframe = document.createElement("iframe");
    this.iframe.className = "pr-photopea-iframe";
    this.iframe.src = "https://www.photopea.com/";

    this.overlay.append(toolbar, this.iframe);
    document.body.appendChild(this.overlay);

    this.iframe.onload = async () => {
      try {
        const dataURL = await this._toDataURL(imageUrl);
        await this._postMessage(`app.open("${dataURL}", null, false);`);
        await this._postMessage(`activeTool("Crop"); activeToolParam("Ratio", 0);`);
      } catch (err) {
        console.error("[Photopea] Failed to load image", err);
      }
    };
  }
  ...
  async _save(saveBtn) {
    ...
    try {
      const responses = await this._postMessage('app.activeDocument.saveToOE("png");');
      const pngPayload = responses[0]; // binary PNG data before the "done" message

      const file = new Blob([pngPayload], { type: "image/png" });
      const filename = "photopea-" + Date.now() + ".png";
      ...
      if (resp.status !== 200) {
        alert("Photopea save failed: " + resp.status + " " + resp.statusText);
        saveBtn.textContent = "Save to Timeline";
      ...
      const subfolder = data.subfolder || "photopea";
      const imgUrl = api.apiURL(`/view?filename=${encodeURIComponent(data.name)}&type=input&subfolder=${encodeURIComponent(subfolder)}`);
      const imageFile = subfolder + "/" + data.name;

      this._onSave?.({ imageFile, imageUrl: imgUrl });
      this.close();
    } catch (err) {
      console.error("[Photopea] Save error", err);
      alert("Photopea save error: " + err.message);
      saveBtn.textContent = "Save to Timeline";
      saveBtn.disabled = false;
    }
  }
  ...
  static showUnavailable() {
    ...
    box.style.cssText = `
      ...
      padding:28px 32px;max-width:460px;width:90%;color:#e0e0e0;
      font-family:inherit;box-shadow:0 8px 32px rgba(0,0,0,0.7);
    `;

    box.innerHTML = `
      <div style="font-size:16px;font-weight:700;margin-bottom:12px;color:#7ec8ff;">
        &#9998; Photopea Not Available
      </div>
      <p style="font-size:13px;line-height:1.6;margin:0 0 14px;">
        <strong>Photopea</strong> is a free, browser-based photo editor
        (similar to Photoshop) used for in-place image editing. It runs
        entirely at <a href="https://www.photopea.com/" target="_blank"
        style="color:#7ec8ff;">photopea.com</a> — no install required.
      </p>
      <p style="font-size:13px;line-height:1.6;margin:0 0 14px;">
        It could not be reached right now. Possible reasons:
      </p>
      <ul style="font-size:13px;line-height:1.8;margin:0 0 18px;padding-left:20px;color:#ccc;">
        <li>No internet connection</li>
        <li>photopea.com is blocked by a firewall or proxy</li>
        <li>The site is temporarily down</li>
      </ul>
      <p style="font-size:12px;color:#888;margin:0 0 18px;">
        To use this feature, ensure your machine can reach
        <code style="background:#2a2a2a;padding:1px 5px;border-radius:3px;">https://www.photopea.com/</code>
        in a browser.
      </p>
    `;
    ...
    closeBtn.style.cssText = `
      display:block;margin:0 auto;padding:7px 28px;
    ...
    closeBtn.onmouseleave = () => closeBtn.style.background = "#2a2a2a";
...
class TimelineEditor {
  constructor(node, container, domWidget) {
    ...
    // Core data
    this.timeline = { segments: [], audioSegments: [], chunkMarkers: [] };
    ...
    this.chunkMarkersWidget = this.node.widgets.find(w => w.name === "chunk_markers");
  ...
  createDOM() {
    ...
    this.replaceFileInput.accept = "image/*";
    this.replaceFileInput.style.display = "none";
    this.replaceFileInput.addEventListener("change", (e) => {
      if (e.target.files?.[0] && this._replaceTargetSegId) {
        this.handleImageReplace(e.target.files[0], this._replaceTargetSegId);
        this._replaceTargetSegId = null;
      }
    ...
    actionGroup.appendChild(this.replaceFileInput);
    ...
    this.wrapper.addEventListener("dragover", (e) => {
      ...
      const isImageTrack = !isAudioTrack && y >= RULER_HEIGHT && y <= RULER_HEIGHT + this.blockHeight;
      const trackType = isAudioTrack ? "audio" : "image";

      // Check if dragging an image file over an existing image segment (replace mode)
      const hasImageFiles = e.dataTransfer.types && (
        e.dataTransfer.types.includes("Files") || e.dataTransfer.types.includes("application/x-moz-file")
      );
      if (isImageTrack && hasImageFiles) {
        const mouseFrameX = x * (totalFrames / logicalWidth);
        const hoverSeg = this.timeline.segments.find(
          s => s.type !== "text" && (s.imageFile || s.imageB64) && mouseFrameX >= s.start && mouseFrameX <= s.start + s.length
        );
        if (hoverSeg) {
          this._dragReplaceTargetId = hoverSeg.id;
          if (this._ghostSegmentId) {
            this._ghostSegmentId = null;
            this._ghostTrack = null;
            this._ghostInitialTimeline = null;
            this._previewSegments = null;
      ...
      this._dragReplaceTargetId = null;
    ...
    this.wrapper.addEventListener("drop", (e) => {
      ...
      const replaceTargetId = this._dragReplaceTargetId;
      this._dragReplaceTargetId = null;
      ...
      if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
        ...
        if (replaceTargetId && imageFiles.length > 0) {
          this.handleImageReplace(imageFiles[0], replaceTargetId);
  ...
  checkResize() {
    // ComfyUI sets a hardcoded inline width on its DOM widget wrapper — clear it so
    // the widget fills the node naturally via CSS width: 100%.
    const widgetWrapper = this.container.parentElement;
    if (widgetWrapper && widgetWrapper.style.width && widgetWrapper.style.width !== '100%') {
    ...
    if (viewportWidth > 0) {
      const newCanvasWidth = Math.max(viewportWidth, viewportWidth * this.zoomLevel);
      const canvasOutOfSync = Math.abs(this.canvas.offsetWidth - newCanvasWidth) > 2;

      if (this._lastWidth !== viewportWidth || this._lastZoom !== this.zoomLevel || this._lastScale !== currentScale || canvasOutOfSync) {
        this._lastWidth = viewportWidth;
        this._lastZoom = this.zoomLevel;
  ...
  // --- Replace Image on Existing Segment (preserves prompt and all other data) ---
  async handleImageReplace(file, segId) {
    if (!file || !file.type.startsWith("image/")) return;
    const seg = this.timeline.segments.find(s => s.id === segId);
    if (!seg) return;
    ...
    try {
      ...
      console.error("[PromptRelay] Image replace failed", err);
    }
  }
  ...
  render() {
    ...
    // --- Chunk Markers ---
    const chunkMarkers = this.timeline.chunkMarkers || [];
    for (const markerFrame of chunkMarkers) {
      const mx = (markerFrame / totalFrames) * width;
      this.ctx.save();
      this.ctx.strokeStyle = "rgba(0, 220, 80, 0.85)";
      this.ctx.lineWidth = 2;
      this.ctx.setLineDash([5, 4]);
      this.ctx.beginPath();
      this.ctx.moveTo(mx, RULER_HEIGHT);
      this.ctx.lineTo(mx, RULER_HEIGHT + this.blockHeight + this.audioTrackHeight);
      this.ctx.stroke();
      this.ctx.setLineDash([]);
      // Small triangle handle at the top of the marker
      this.ctx.fillStyle = "rgba(0, 220, 80, 0.9)";
      this.ctx.beginPath();
      this.ctx.moveTo(mx - 6, RULER_HEIGHT);
      this.ctx.lineTo(mx + 6, RULER_HEIGHT);
      this.ctx.lineTo(mx, RULER_HEIGHT + 9);
      this.ctx.closePath();
      this.ctx.fill();
      this.ctx.restore();
    }

    // --- Drag-Replace Highlight ---
    if (this._dragReplaceTargetId) {
      const replaceSeg = this.timeline.segments.find(s => s.id === this._dragReplaceTargetId);
      if (replaceSeg) {
        const rx = (replaceSeg.start / totalFrames) * width;
        const rw = (replaceSeg.length / totalFrames) * width;
        this.ctx.save();
        this.ctx.strokeStyle = "rgba(0, 200, 80, 0.9)";
        this.ctx.lineWidth = 3;
        this.ctx.strokeRect(rx + 1, RULER_HEIGHT + 1, rw - 2, this.blockHeight - 2);
        this.ctx.fillStyle = "rgba(0, 200, 80, 0.15)";
        this.ctx.fillRect(rx + 1, RULER_HEIGHT + 1, rw - 2, this.blockHeight - 2);
        this.ctx.fillStyle = "rgba(0, 200, 80, 0.9)";
        this.ctx.textAlign = "center";
        this.ctx.textBaseline = "middle";
        this.ctx.font = "bold 13px sans-serif";
        this.ctx.fillText("Replace Image", rx + rw / 2, RULER_HEIGHT + this.blockHeight / 2);
        this.ctx.restore();
      }
    }
  ...
  commitChanges(skipRender = false) {
    ...
    const toSave = {
      ...
      chunkMarkers: (this.timeline.chunkMarkers || []).slice().sort((a, b) => a - b)
    };

    const jsonStr = JSON.stringify(toSave);
    if (this.timelineDataWidget) this.timelineDataWidget.value = jsonStr;

    if (this.chunkMarkersWidget) {
      this.chunkMarkersWidget.value = (this.timeline.chunkMarkers || []).slice().sort((a, b) => a - b).join(",");
    }
  ...
  onContextMenu(e) {
    ...
    // Check if clicking near a chunk marker line
    if (!clickedSeg && (isAudioTrack || isImageTrack)) {
      const MARKER_HIT_PX = 6;
      const markers = this.timeline.chunkMarkers || [];
      for (let mi = 0; mi < markers.length; mi++) {
        const mx = (markers[mi] / totalFrames) * logicalWidth;
        if (Math.abs(mouseX - mx) <= MARKER_HIT_PX) {
          this.showChunkMarkerContextMenu(e.clientX, e.clientY, mi);
          return;
        }
      }
    }
    ...
    if (clickedSeg) {
      this.showContextMenu(e.clientX, e.clientY, clickedSeg, trackType, cursor);
  ...
  showContextMenu(clientX, clientY, seg, trackType, clickedFrame = null) {
    ...
    if (isImage) {
      ...
      replaceBtn.onclick = () => {
        this._replaceTargetSegId = seg.id;
      ...
      photopeaBtn.className = "pr-gap-menu-btn";
      photopeaBtn.style.color = "#7ec8ff";
      photopeaBtn.innerHTML = `&#9998; Edit in Photopea`;
      photopeaBtn.onclick = () => {
        this.dismissContextMenu();
        const dlg = new PhotopeaDialog();
        dlg.open(seg.imageB64, ({ imageFile, imageUrl }) => {
          seg.imageFile = imageFile;
    ...
    if (trackType !== "audio") {
      const chunkDivider = document.createElement("hr");
      chunkDivider.className = "pr-settings-divider";
      menu.appendChild(chunkDivider);

      const chunkBtn = document.createElement("button");
      chunkBtn.className = "pr-gap-menu-btn";
      chunkBtn.innerHTML = `&#9986; Add Chunk Split`;
      chunkBtn.style.color = "#00dc50";
      chunkBtn.onclick = () => {
        const leftEdge = seg.start;
        const rightEdge = seg.start + seg.length;
        let snapFrame;
        if (clickedFrame !== null) {
          const distLeft = Math.abs(clickedFrame - leftEdge);
          const distRight = Math.abs(clickedFrame - rightEdge);
          snapFrame = distLeft <= distRight ? leftEdge : rightEdge;
        } else {
          snapFrame = Math.round((leftEdge + rightEdge) / 2);
        }
        snapFrame = Math.round(snapFrame);
        if (!this.timeline.chunkMarkers) this.timeline.chunkMarkers = [];
        if (!this.timeline.chunkMarkers.includes(snapFrame)) {
          this.timeline.chunkMarkers.push(snapFrame);
          this.commitChanges();
          this.renderTimeline();
        }
        this.dismissContextMenu();
      };
      menu.appendChild(chunkBtn);
    }
  ...
  showGapContextMenu(clientX, clientY, gap) {
    ...
    if (currentTrack === "image") {
      ...
      chunkBtn.style.color = "#00dc50";
      chunkBtn.onclick = () => {
        const markerFrame = Math.round(gap.clickedFrame !== undefined ? gap.clickedFrame : (gap.frameStart + gap.frameEnd) / 2);
        if (!this.timeline.chunkMarkers) this.timeline.chunkMarkers = [];
        if (!this.timeline.chunkMarkers.includes(markerFrame)) {
          this.timeline.chunkMarkers.push(markerFrame);
          this.commitChanges();
        }
        this.dismissContextMenu();
      };
  ...
  showChunkMarkerContextMenu(clientX, clientY, markerIdx) {
    this.dismissContextMenu();
    const menu = document.createElement("div");
    menu.className = "pr-gap-menu";
    menu.style.left = `${clientX + 6}px`;
    ...
    label.style.cssText = "padding: 4px 10px; font-size: 10px; color: #00dc50; border-bottom: 1px solid #333; margin-bottom: 2px;";
    label.textContent = `Chunk Split — ${this.formatTime(this.timeline.chunkMarkers[markerIdx])}`;
    menu.appendChild(label);

    const delBtn = document.createElement("button");
    delBtn.className = "pr-gap-menu-btn";
    delBtn.innerHTML = "Delete Chunk Split";
    delBtn.style.color = "#ff6666";
    delBtn.onclick = () => {
      this.timeline.chunkMarkers.splice(markerIdx, 1);
      this.commitChanges();
      this.dismissContextMenu();
    };
    menu.appendChild(delBtn);
  ...
  showGapMenu(clientX, clientY, gap) {
    ...
    if (currentTrack === "image") {
      ...
      chunkBtn.addEventListener("click", () => {
        const markerFrame = Math.round((gap.frameStart + gap.frameEnd) / 2);
        if (!this.timeline.chunkMarkers) this.timeline.chunkMarkers = [];
        if (!this.timeline.chunkMarkers.includes(markerFrame)) {
          this.timeline.chunkMarkers.push(markerFrame);
          this.commitChanges();
...
app.registerExtension({
  name: "CoachBateLTXDirector",
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    if (nodeData.name === "CoachBateLTXDirector") {
