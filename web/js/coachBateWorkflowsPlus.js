...
import {
    ...
    rewritePrefix,
    isInTrash,
    isBrowsableWorkflowFile,
    TRASH_FOLDER,
} from "./workflows_plus/pathUtils.js";

const WF_ROOT = "workflows"; // userdata subdir
const ENABLED_SETTING_ID = "CoachBate.WorkflowsPlus.Enabled";
...
const S = {
    ...
    // Custom nodes virtual tree (read-only)
    cnRootOpen: false,
    cnExpanded: new Set(),
    cnCache: new Map(), // cn-relative folder path → { folders, files, loading, error }
};
...
.wfp-ctxmenu { position:fixed; z-index:10020; background: var(--comfy-menu-bg);
...
.wfp-cn-sep { border-top:1px solid var(--border-color); cursor:default; margin-top:2px; }
.wfp-cn-sep:hover { background:transparent; }
.wfp-cn-row .wfp-name { opacity:0.85; }
.wfp-cn-status { opacity:0.5; font-style:italic; }
.wfp-modal-overlay { position:fixed; inset:0; z-index:10030; background:rgba(0,0,0,0.5);
...
function rawToEntry(item) {
    ...
    // ComfyUI's /userdata listing returns every file unfiltered — only
    // workflow JSON and PNGs (which can carry an embedded workflow) belong
    // in the panel; desktop.ini, .bak backups, thumbs.db, etc. are dropped.
    if (!isBrowsableWorkflowFile(key)) return null;
...
function currentRows() {
    ...
    if (!S.tree) return { mode: "tree", rows: cnRows() };
    const wfRows = flattenVisible(S.tree, S.expanded, S.sortSpec, getPinnedKeysForFolder);
    return { mode: "tree", rows: [...wfRows, ...cnRows()] };
}
...
function renderRows() {
    ...
    S.vlist.renderRow = mode === "search" ? renderSearchRow : renderMixedTreeRow;
...
function renderTreeRow(item, rowEl) {
    ...
    // No chevron/file-icon spacer here (unlike folders, which need the
    // chevron column): the pin marker is the only fixed-width prefix, so a
    // file's name starts at the same x-position as a folder's name at the
    // same depth (both have a single 14px column before the name) instead
    // of trailing extra icon columns folders don't have.
    rowEl.innerHTML =
        pinIconHtml(item.pinned) +
        `<span class="wfp-name"></span>` +
...
async function openWorkflowByKey(key) {
    ...
    try {
        ...
        if (key.toLowerCase().endsWith(".png")) {
            // Not a raw-JSON workflow file — fetch the image and hand it to
            // the same extraction path used for drag-and-drop, which knows
            // how to pull an embedded workflow out of a PNG's metadata.
            const resp = await api.getUserData(fullPath);
            if (!resp.ok) throw new Error(`load failed: ${resp.status}`);
            const blob = await resp.blob();
            const file = new File([blob], key.split("/").pop(), { type: "image/png" });
            if (typeof app.handleFile !== "function") {
                throw new Error("app.handleFile is missing in this frontend version");
            }
...
// ── Auto-refresh on external workflow mutations ──────────────────────────────
// ComfyUI's frontend dispatches no event for workflow save/rename/delete —
// nothing lands on `api` for userdata writes, so there is nothing to
// addEventListener to. What it does have is a single choke point: every
// save / save-as / rename / delete, from any UI (Ctrl+S, the topbar tab menu,
// the stock Workflows sidebar, or this panel), ends up calling the matching
// method on the Pinia workflow store at `app.extensionManager.workflow` —
// the workflowService layer above it just delegates down. So we wrap those
// four methods once and refresh off the back of them.
//
// ⚠️ Frontend-internal surface: re-verify on comfyui_frontend_package
// upgrades. Each wrap is individually guarded, and a missing method only
// disables auto-refresh for that one operation (Refresh still works).
...
function scheduleAutoRefresh() {
    ...
    _refreshTimer = setTimeout(() => {
        _refreshTimer = null;
        if (!S.el) {
            // Panel is closed — don't spend a listing fetch on a tree nobody
            // is looking at. Expiring the cache makes renderPanel refetch on
            // the next open instead.
            S.lastFetchMs = 0;
            return;
        }
        // rebuildIndex no-ops while a fetch is already in flight, so re-arm
        // rather than silently dropping this refresh.
        if (S.fetching) { scheduleAutoRefresh(); return; }
        // sync:false — the store just mutated itself and is already current;
        // all we need is a fresh on-disk listing.
...
function installStoreHooks() {
    ...
    if (store._cbWfpHooked) return true;

    const wrap = (name, hooks = {}) => {
        const orig = store[name];
        if (typeof orig !== "function") {
            console.warn(`[Workflows+] workflow store has no ${name}() — auto-refresh for that operation is off`);
            return;
        }
        store[name] = function (...args) {
            let captured;
            try { captured = hooks.before?.(...args); }
            catch (err) { console.warn(`[Workflows+] ${name} pre-hook failed`, err); }
            const result = orig.apply(this, args);
            const finish = () => {
                try { hooks.after?.(captured, ...args); }
                catch (err) { console.warn(`[Workflows+] ${name} post-hook failed`, err); }
                scheduleAutoRefresh();
            };
            // Settled either way: a failed save still leaves the listing worth
            // re-reading, and we must never swallow the caller's rejection.
    ...
    wrap("saveWorkflow");
    wrap("saveAs");
    // Rename and delete also have to carry this panel's own per-path state
    // (pins, MRU) across — the panel's own commitRename/deleteFiles already do
    // this, but a rename or delete driven from ComfyUI's UI would otherwise
    // leave a pin/MRU entry pointing at a path that no longer exists. Both
    // migrations are idempotent, so the double call on panel-driven ops is a
    // no-op the second time.
    wrap("renameWorkflow", {
        // The store renames in place: wf.path is the old path on the way in,
        // and the new one once the promise settles.
        before: (wf) => stripRoot(wf?.path),
        after: (oldKey, wf) => {
            const newKey = stripRoot(wf?.path);
            if (!oldKey || !newKey || oldKey === newKey) return;
            pinsMigrateKey(oldKey, newKey);
            S.mru = mruMigrateKey(oldKey, newKey);
        },
    });
    wrap("deleteWorkflow", {
        before: (wf) => stripRoot(wf?.path),
        after: (oldKey) => {
            if (!oldKey) return;
            pinsMigrateKey(oldKey, null);
            S.mru = mruMigrateKey(oldKey, null);
        },
    });

    store._cbWfpHooked = true;
    return true;
}

// The workflow store is created during frontend boot; extension setup() can
// win that race, so retry briefly before giving up.
function startStoreHooks(attempt = 0) {
    if (installStoreHooks()) return;
    if (attempt >= 20) {
        console.warn("[Workflows+] workflow store never appeared — auto-refresh disabled, use the Refresh button");
        return;
    }
    setTimeout(() => startStoreHooks(attempt + 1), 250);
}
...
function setSearchMode(mode) {
    if (S.searchMode === mode) {
        // Already active — act as refresh.
        const query = S.searchInput ? S.searchInput.value : "";
        if (mode === "contains") {
            if (query.trim()) {
                S.containsQuery = query;
                runContainsSearch(S.containsQuery);
            }
        } else {
            rebuildIndex({ sync: true });
        }
        return;
    }
    cancelContainsSearch();
    // Carry the current text box value into the new mode's query field so the
    // user doesn't lose what they typed.
    const currentText = S.searchInput ? S.searchInput.value : "";
    S.searchMode = mode;
    S.containsResults = null;
    if (mode === "contains") {
        S.containsQuery = currentText;
    } else {
        S.searchText = currentText;
    }
    updateModeButtons();
    updateSearchPlaceholder();
    if (S.searchInput) S.searchInput.value = currentText;
    if (mode === "contains" && currentText.trim()) {
        runContainsSearch(S.containsQuery);
    } else {
        renderRows();
    }
...
function wireDragAndDrop(root) {
    let hint = null;
    let watchdog = null;
    // dragleave/dragend are unreliable across browsers when a drag is
    // cancelled or released outside the tracked element (e.g. dropped
    // outside the window, or Escape pressed mid-drag) — none of those
    // reliably fire on `root`, which used to leave the hint stuck forever
    // since nothing else ever calls clearHint() again. dragover fires
    // continuously (every animation frame-ish) while a real drag is still
    // in progress, so use its absence as a fallback signal that the drag
    // ended.
    const armWatchdog = () => {
        clearTimeout(watchdog);
        watchdog = setTimeout(clearHint, 400);
    };
    const showHint = () => {
    ...
    root.addEventListener("dragleave", (e) => {
        if (!root.contains(e.relatedTarget)) clearHint();
    });
...
// ── Custom nodes virtual tree (read-only) ─────────────────────────────────────
// Shows a "📦 custom_nodes" root at the bottom of the workflow tree.
// Only .json files that pass the workflow heuristic (have a "nodes" list) are
// shown — no package.json, pyproject.toml, or other config files.
// Contents of each folder are fetched lazily on first expand.

async function fetchCnFolder(folderPath) {
    if (S.cnCache.has(folderPath) && !S.cnCache.get(folderPath).error) return;
    S.cnCache.set(folderPath, { folders: [], files: [], loading: true, error: null });
    renderRows();
    try {
        const params = new URLSearchParams();
        if (folderPath) params.set("folder", folderPath);
        const resp = await api.fetchApi(`/coachbate/custom_nodes/listing?${params}`);
        if (!resp.ok) throw new Error(`listing failed: ${resp.status}`);
        ...
        const files = data.files.map((item) => ({
            ...
            kind: "cn-file",
            path: item.path,
            name: item.path.split("/").pop().replace(/\.[^.]+$/, ""),
            size: item.size,
            modified: item.modified ?? 0,
            created: item.created ?? 0,
        }));
        S.cnCache.set(folderPath, { folders: data.folders ?? [], files, loading: false, error: null });
    } catch (err) {
        console.error("[Workflows+] cn listing failed", folderPath, err);
        S.cnCache.set(folderPath, { folders: [], files: [], loading: false, error: String(err?.message || err) });
    }
    renderRows();
}

function toggleCnRoot() {
    S.cnRootOpen = !S.cnRootOpen;
    if (S.cnRootOpen && !S.cnCache.has("")) {
        fetchCnFolder("");
    } else {
        renderRows();
    }
...
function toggleCnFolder(folderPath) {
    if (S.cnExpanded.has(folderPath)) {
        S.cnExpanded.delete(folderPath);
        renderRows();
    } else {
        S.cnExpanded.add(folderPath);
        if (!S.cnCache.has(folderPath)) {
            fetchCnFolder(folderPath);
        } else {
            renderRows();
        }
    }
...
function appendCnFolderContents(folderPath, childDepth, rows) {
    const cached = S.cnCache.get(folderPath);
    if (!cached) return;
    if (cached.loading) {
        rows.push({ source: "cn", kind: "cn-status", text: "Loading…", depth: childDepth });
        return;
    }
    if (cached.error) {
        rows.push({ source: "cn", kind: "cn-status", text: `⚠ ${cached.error}`, depth: childDepth });
        return;
    }
    for (const subPath of cached.folders) {
        const name = subPath.split("/").pop();
        const open = S.cnExpanded.has(subPath);
        rows.push({ source: "cn", kind: "cn-folder", path: subPath, name, open, depth: childDepth });
        if (open) appendCnFolderContents(subPath, childDepth + 1, rows);
    }
    for (const f of cached.files) {
        rows.push({ ...f, depth: childDepth });
    }
}

function cnRows() {
    const rows = [];
    rows.push({ source: "cn", kind: "cn-sep", depth: 0 });
    rows.push({ source: "cn", kind: "cn-root", open: S.cnRootOpen, depth: 0 });
    if (S.cnRootOpen) appendCnFolderContents("", 1, rows);
    return rows;
}

function renderCnRow(item, rowEl) {
    rowEl.oncontextmenu = null;
    rowEl.draggable = false;
    rowEl.ondragstart = null;
    rowEl.ondragover = null;
    ...
    if (item.kind === "cn-sep") {
        rowEl.className = "wfp-row wfp-cn-sep";
    ...
    if (item.kind === "cn-root") {
        rowEl.className = "wfp-row wfp-folder wfp-cn-row";
        rowEl.innerHTML =
            `<span class="wfp-chevron">${item.open ? "▾" : "▸"}</span>` +
            `<span class="wfp-icon">📦</span>` +
            `<span class="wfp-name">custom_nodes</span>`;
        rowEl.onclick = toggleCnRoot;
        return;
    }

    if (item.kind === "cn-folder") {
        rowEl.className = "wfp-row wfp-folder wfp-cn-row";
        rowEl.innerHTML =
            `<span class="wfp-chevron">${item.open ? "▾" : "▸"}</span>` +
            `<span class="wfp-name"></span>`;
        rowEl.querySelector(".wfp-name").textContent = item.name;
        rowEl.title = item.path;
        rowEl.onclick = () => toggleCnFolder(item.path);
        return;
    }

    if (item.kind === "cn-status") {
        rowEl.className = "wfp-row wfp-cn-row";
        rowEl.innerHTML = `<span class="wfp-name wfp-cn-status"></span>`;
        rowEl.querySelector(".wfp-name").textContent = item.text;
        return;
    }
    ...
    rowEl.innerHTML =
        ...
        `<span class="wfp-icon">◈</span>` +
        `<span class="wfp-name"></span>` +
        `<span class="wfp-date">${fmtDate(item.modified)}</span>`;
    rowEl.querySelector(".wfp-name").textContent = item.name;
    rowEl.title = item.path;
    rowEl.onclick = () => openCnWorkflow(item.path);
}

function renderMixedTreeRow(item, rowEl) {
    if (item.source === "cn") renderCnRow(item, rowEl);
    else renderTreeRow(item, rowEl);
}

async function openCnWorkflow(cnPath) {
    try {
        const resp = await api.fetchApi(`/coachbate/custom_nodes/file?path=${encodeURIComponent(cnPath)}`);
        if (!resp.ok) throw new Error(`load failed: ${resp.status}`);
        const json = await resp.json();
        await app.loadGraphData(json, true, true, cnPath);
    } catch (err) {
        console.error("[Workflows+] cn open failed", cnPath, err);
        toast("error", "Could not open workflow", String(err?.message || err));
    }
}
...
function destroyPanel() {
    ...
    // Start fresh next time the panel opens — nothing about "where you were
    // browsing" should silently carry over. Sort order and the "search
    // subfolders" toggle are real preferences (persisted to localStorage)
    // and are left alone; everything below is just transient browsing state.
    S.searchText = "";
    S.searchMode = "name";
    S.containsQuery = "";
    S.containsResults = null;
    ...
    settings: [
        {
            ...
            category: ["CoachBate", "Workflows+"],
            name: "Enable Workflows+ panel",
            type: "boolean",
            defaultValue: true,
            sortOrder: 300,
            tooltip: "Show the Workflows+ sidebar panel. Reload ComfyUI after changing this setting.",
        },
    ],
    async setup() {
