 ...
 * folder (folders always sorted A-Z), emit its pinned files (sorted by
 * sortSpec), then its unpinned files (sorted by sortSpec), then its
 * subfolders (recursing into expanded ones).
 *
 * Files are listed before subfolders at the same depth deliberately: a
 * folder row can visually read as "containing" whatever equally-indented
 * rows follow it (especially when collapsed), but nothing reads as being
 * "inside" a file that precedes it. Putting files first keeps a folder's
 * own direct files from looking like they belong to whichever (possibly
 * collapsed) subfolder happens to be rendered just above them.
 *
