...
// ComfyUI's built-in /userdata listing returns every file under the
// workflows folder unfiltered — desktop.ini, .bak backups, thumbs.db,
// editor swap files, etc. all show up alongside real workflows unless the
// panel filters them out itself.
const BROWSABLE_EXTENSIONS = new Set([".json", ".png"]);

/**
 * True if `key` is a file type the Workflows+ panel should list: a raw
 * workflow (.json) or an image that can carry an embedded workflow (.png).
 * Everything else (desktop.ini, .bak, thumbs.db, ...) is filtered out.
 */
export function isBrowsableWorkflowFile(key) {
    return BROWSABLE_EXTENSIONS.has(extOf(key).toLowerCase());
}
