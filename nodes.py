...
In increment/decrement mode, the frontend queues the next shot only after the
whole prompt succeeds. Leave native Auto Queue disabled. Fixed mode runs once.
...
---------------
  ...
  video_continuation    — JSON boolean (default false)
  continuation_source_shot_id — stable source shot ID when continuing; no output path
  continuation_context_frames — retained AV prefix, 39/90/141/... frames (default 39)

  lora_tag              — portable LoRA tag; used for global_prompt when that
                          field is absent. An explicit empty global_prompt is
                          preserved so a workflow can opt into voice-only mode.

  reference_image_1 … reference_image_6
                        — ordered H3 image references; aliases for the six
                          h3_ref_image_N outputs
  reference_audio_1 … reference_audio_3
                        — ordered H3 audio references; aliases for the three
                          h3_ref_audio_N outputs
  reference_audio_1_start_seconds … reference_audio_3_start_seconds
  reference_audio_1_end_seconds … reference_audio_3_end_seconds
                        — optional source-audio slice bounds for each H3 audio
                          reference; end defaults to -1 (through end of file)

  reference_video_1 … reference_video_3
                        — video-reference paths; blank omits the slot
  reference_video_1_settings … reference_video_3_settings
                        — per-slot VHS clip settings objects, serialized for
                          CoachBateLoadH3VideoPath; source audio defaults off
...
import threading
import time
from .nodes_h3_video_path import resolve_video_references
...
class CoachBateShotLoader(_NodeBase):
    ...
    RETURN_TYPES  = ("STRING",       "INT",               "STRING",   "STRING",               "STRING",      "STRING",    "STRING",             "STRING",           "STRING",               "INT",          "FLOAT",                 "FLOAT",              "FLOAT",               "STRING",        "STRING",       "STRING",         "STRING",          "STRING",          "STRING",          "STRING",          "STRING",          "STRING",          "STRING",          "STRING",          "STRING",          "FLOAT",                            "FLOAT",                          "FLOAT",                            "FLOAT",                          "FLOAT",                            "FLOAT")
    RETURN_NAMES  = ("video_prompt", "duration_seconds",  "shot_id",  "video_filename_prefix", "start_image", "end_image", "start_image_prompt", "negative_prompt",  "negative_audio_prompt", "total_shots",  "start_image_strength",  "end_image_strength", "end_image_timestamp", "global_prompt", "local_prompts", "segment_lengths", "h3_ref_image_1", "h3_ref_image_2", "h3_ref_image_3", "h3_ref_image_4", "h3_ref_image_5", "h3_ref_image_6", "h3_ref_audio_1", "h3_ref_audio_2", "h3_ref_audio_3", "h3_ref_audio_1_start_seconds", "h3_ref_audio_1_end_seconds", "h3_ref_audio_2_start_seconds", "h3_ref_audio_2_end_seconds", "h3_ref_audio_3_start_seconds", "h3_ref_audio_3_end_seconds")
    RETURN_TYPES += ("STRING",)
    RETURN_NAMES += ("shot_payload",)
    RETURN_TYPES += ("BOOLEAN", "STRING", "INT")
    RETURN_NAMES += ("video_continuation", "continuation_source_shot_id", "continuation_context_frames")
    RETURN_TYPES += ("STRING",) * 6
    RETURN_NAMES += tuple(f"h3_ref_video_{i}" for i in range(1, 4))
    RETURN_NAMES += tuple(f"h3_ref_video_{i}_settings" for i in range(1, 4))
    FUNCTION      = "execute"
    CATEGORY      = "CoachBate"
    # ⚠️ NOT an OUTPUT_NODE, deliberately.
    #
    # validate_prompt() adds every OUTPUT_NODE=True node to the execution set
    # (ComfyUI execution.py), so the flag made this node run on EVERY queue
    # press no matter what it was wired to — including when its output fed a
    # dead branch that produced nothing.  Pressing Run on such a workflow
    # advanced the sequence and fired a whole batch for no reason.
    #
    # Without the flag the node runs only when it is an ancestor of a real
    # output node, which is the same rule every ordinary node follows.  The
    # status readout is unaffected: get_output_from_returns() collects the "ui"
    # dict from any executed node's return value, output node or not.
    DESCRIPTION   = (
        ...
        "Set batch_enabled=false to retain a record without rendering it. The frontend advances after successful completion; leave native Auto Queue off."
    )
    RETURN_TOOLTIPS = (
        ...
        "Absolute path for MiniMax H3 reference image slot 1. Usually the location reference.",
        "Absolute path for MiniMax H3 reference image slot 2. Usually character reference sheet 1.",
        "Absolute path for MiniMax H3 reference image slot 3. Usually character reference sheet 2.",
        "Absolute path for MiniMax H3 reference image slot 4. Usually character reference sheet 3 or a prop/outfit reference.",
        "Absolute path for MiniMax H3 reference image slot 5.",
        "Absolute path for MiniMax H3 reference image slot 6.",
        "Absolute path for MiniMax H3 reference audio slot 1.",
        "Absolute path for MiniMax H3 reference audio slot 2.",
        "Absolute path for MiniMax H3 reference audio slot 3.",
        "Start time in seconds for MiniMax H3 reference audio slot 1.",
        "End time in seconds for MiniMax H3 reference audio slot 1; -1 uses the rest of the file.",
        "Start time in seconds for MiniMax H3 reference audio slot 2.",
        "End time in seconds for MiniMax H3 reference audio slot 2; -1 uses the rest of the file.",
        "Start time in seconds for MiniMax H3 reference audio slot 3.",
        "End time in seconds for MiniMax H3 reference audio slot 3; -1 uses the rest of the file.",
        "Selected JSON record and parsed values for a per-shot expanding workflow.",
        "Continue a previous shot when true; defaults to false for existing shotlists.",
        "Stable source shot ID, resolved through the workflow run's output manifest, never a guessed filename.",
        "Preserved H3 prefix frames, default 39 (1.625 seconds at 24 fps).",
        "Video reference slot 1 path; blank omits this reference.",
        "Video reference slot 2 path; blank omits this reference.",
        "Video reference slot 3 path; blank omits this reference.",
        "Serialized VHS clip settings for video reference slot 1.",
        "Serialized VHS clip settings for video reference slot 2.",
        "Serialized VHS clip settings for video reference slot 3.",
    )
    ...
    if not _V3:
        ...
        def IS_CHANGED(cls, mode, **kwargs):
            if mode != "fixed":
                return time.time()
            return cls._shotlist_fingerprint(kwargs.get("json_path", ""))
    ...
    def define_schema(cls):
        ...
        return io.Schema(
            ...
            outputs=[
                ...
                io.String.Output("h3_ref_image_1"),
                io.String.Output("h3_ref_image_2"),
                io.String.Output("h3_ref_image_3"),
                io.String.Output("h3_ref_image_4"),
                io.String.Output("h3_ref_image_5"),
                io.String.Output("h3_ref_image_6"),
                io.String.Output("h3_ref_audio_1"),
                io.String.Output("h3_ref_audio_2"),
                io.String.Output("h3_ref_audio_3"),
                io.Float.Output("h3_ref_audio_1_start_seconds"),
                io.Float.Output("h3_ref_audio_1_end_seconds"),
                io.Float.Output("h3_ref_audio_2_start_seconds"),
                io.Float.Output("h3_ref_audio_2_end_seconds"),
                io.Float.Output("h3_ref_audio_3_start_seconds"),
                io.Float.Output("h3_ref_audio_3_end_seconds"),
                io.String.Output("shot_payload"),
                io.Boolean.Output("video_continuation"),
                io.String.Output("continuation_source_shot_id"),
                io.Int.Output("continuation_context_frames"),
                *[io.String.Output(f"h3_ref_video_{i}") for i in range(1, 4)],
                *[io.String.Output(f"h3_ref_video_{i}_settings") for i in range(1, 4)],
            ],
            # ⚠️ Deliberately NOT an output node — same reason as the V1
            # OUTPUT_NODE comment above: an output node executes on every queue
            # press even when its outputs feed nothing. Keep the two schemas in
            # agreement; flipping only one of them reintroduces the bug on
            # whichever API path the server happens to use.
        )
    ...
    @staticmethod
    def _shotlist_fingerprint(json_path):
        try:
    ...
    def execute(cls, json_path: str, shot_number: int, mode: str, framerate: int = 24):
        ...
        active = [i for i, s in enumerate(shots)
                  ...
                  and s.get("batch_enabled", True) is not False]

        if not active:
            raise ValueError("[CoachBate] No enabled unfinished shots remain.")

        # --- resolve index, skipping DONE shots ---
        with _state_lock:
            path_key = os.path.normcase(os.path.abspath(json_path))
            if _shot_loader_state.get("json_path") != path_key:
                _shot_loader_state.update(json_path=path_key, _seeded=False, stored_index=shot_index)
            if mode == "fixed":
            ...
            _shot_pre = shots[idx]
            from .nodes_shot_continuity import continuation_settings, require_ready
            require_ready(_shot_pre)
            continuation = continuation_settings(_shot_pre)
        ...
        # --- extract fields ---
        global_prompt         = str(shot.get("global_prompt", shot.get("lora_tag", "")))
        local_prompts         = str(shot.get("local_prompts", ""))
        video_prompt          = str(shot.get("video_prompt", "")).strip()
        if not video_prompt:
            video_prompt = "\n\n".join(part for part in (global_prompt.strip(), local_prompts.strip()) if part)
        ...
        h3_ref_images         = cls._resolve_h3_references(shot, "image", 6)
        h3_ref_audios         = cls._resolve_h3_references(shot, "audio", 3)
        h3_ref_audio_ranges   = cls._resolve_h3_audio_ranges(shot, 3)
        ...
        outputs += (json.dumps({"shot": shot, "loader_outputs": outputs,
                               "shotlist_path": os.path.abspath(json_path)}, ensure_ascii=False),)
        outputs += continuation
        outputs += resolve_video_references(shot)
    ...
    @staticmethod
    def _resolve_h3_references(shot: dict, media_type: str, count: int) -> tuple[str, ...]:
        """Resolve flat H3 fields, with optional array and friendly-alias fallbacks."""
        array_key = f"h3_reference_{media_type}s"
        values = shot.get(array_key, [])
        ...
        legacy_aliases = legacy_image_aliases if media_type == "image" else legacy_audio_aliases

        resolved = []
        for index in range(count):
            flat_key = f"h3_ref_{media_type}_{index + 1}"
            value = shot.get(flat_key)
            if value in (None, ""):
                value = shot.get(f"reference_{media_type}_{index + 1}")
            if value in (None, "") and index < len(values):
                value = values[index]
            if value in (None, "") and index < len(legacy_aliases):
                value = shot.get(legacy_aliases[index], "")
            resolved.append(str(value or "").strip())
        return tuple(resolved)

    @staticmethod
    def _resolve_h3_audio_ranges(shot: dict, count: int) -> tuple[float, ...]:
        """Resolve ordered source-audio slice bounds for the H3 audio slots."""
        resolved = []
        for index in range(1, count + 1):
            for suffix, default in (("start_seconds", 0.0), ("end_seconds", -1.0)):
                key = f"reference_audio_{index}_{suffix}"
                value = shot.get(key, default)
                try:
                    resolved.append(float(value))
                except (TypeError, ValueError):
                    log.warning("[CoachBate] Invalid %s value '%s', defaulting to %s", key, value, default)
...
class CoachBateBatchPrompter:
        ...
        return {
            "required": {
                ...
                "multiline_text": ("STRING", {
                    ...
                    "tooltip": (
                        "One prompt per line (blank lines are ignored). Turn "
                        "newline_delimiter OFF to separate prompts with \"||\" "
                        "instead, so one prompt can span several lines."
                    ),
                }),
                "append_text": ("STRING", {
                    "multiline": False,
            ...
            "optional": {
                ...
                "newline_delimiter": ("BOOLEAN", {
                    "default": True,
                    "tooltip": (
                        "ON — New line delimiter (default):\n"
                        "Every non-blank line is its own prompt.\n\n"
                        "OFF — Double pipe delimiter:\n"
                        "Prompts are separated by \"||\" instead, so a single prompt can "
                        "span as many lines as you like. Useful for the very long prompts "
                        "newer models take, where flattening each one onto a single line "
                        "is tedious. The gutter numbering updates to match either way."
                    ),
                }),
            },
        }
    ...
    def execute(
        ...
        #
        # Everything below works on a list of *segments*, one per potential
        # prompt.  With newline_delimiter ON that's one segment per line (the
        # original behaviour); OFF, segments are split on "||" so a single
        # prompt may span many lines.  Blank/whitespace-only segments are
        # skipped either way, so the rest of the logic is identical — only the
        # split differs.  Keep this in sync with _promptSegments() in
        # web/js/coachBateBatchPrompter.js, which numbers the gutter.
        lines = multiline_text.split('\n' if newline_delimiter else '||')
